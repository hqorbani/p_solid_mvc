def cover(func):
    def run_cover(*args , **kwargs):
        result = {}
        result['welcome_message'] = 'welcome to program...'
        result['plans'] = func(*args , **kwargs)
        result['end_message'] = 'end program...'
        return result
    return run_cover
@cover
...
result = study_plan_generator(grades, target)
for key , value in result.items():
    if key == 'plans':
        for grade , plan in value.items():
            print(grade , plan)
    else:
        print(value)
